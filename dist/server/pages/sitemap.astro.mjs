import { a as createComponent, r as renderComponent, b as renderTemplate, m as maybeRenderHead, e as addAttribute } from '../chunks/astro/server_CROlnyxU.mjs';
import 'kleur/colors';
import { g as getCollection } from '../chunks/_astro_content_tm8j9PMM.mjs';
import { a as $$PageLayout } from '../chunks/PageLayout_Byy34s9y.mjs';
export { renderers } from '../renderers.mjs';

const $$Sitemap = createComponent(async ($$result, $$props, $$slots) => {
  const normalizeBlogSlug = (slug) => slug.replace(/^\/?blog\/?/i, "").replace(/^\/+|\/+$/g, "");
...
</p> <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> <!-- Pagine Statiche --> <div> <h2 class="text-2xl font-semibold mb-4">Pagine Principali</h2> <ul class="space-y-2"> ${staticPages.map((page) => renderTemplate`<li> <a${addAttribute(page.url, "href")} class="text-blue-600 hover:underline">${page.title}</a> </li>`)} </ul> </div> <!-- Blog --> <div> <h2 class="text-2xl font-semibold mb-4">Articoli del Blog</h2> <ul class="space-y-2"> ${posts.map((post) => {
    const slug = normalizeBlogSlug(post.data.slug || post.id);
    return renderTemplate`<li> <a${addAttribute(`/blog/${slug}`, "href")} class="text-blue-600 hover:underline"> ${post.data.title} </a> </li>`;
  })} </ul> </div> <!-- Portfolio --> <div> <h2 class="text-2xl font-semibold mb-4">Portfolio</h2> <ul class="space-y-2"> ${portfolioItems.map((item) => renderTemplate`<li> <a${addAttribute(`/portfolio/${item.id}`, "href")} class="text-blue-600 hover:underline"> ${item.data.title} </a> </li>`)} </ul> </div> </div> </section> ` })}`;
