  ...
  if (!completed || !result) {
    return (
      <div className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm md:p-8 dark:border-neutral-800 dark:bg-neutral-900/60">
        ...
        <div className="mt-3 h-2 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800" role="progressbar" aria-label="Avanzamento valutazione" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}>
          <div className="h-full rounded-full bg-violet-600 transition-all" style={{ width: `${progress}%` }} />
        </div>

        <h2 ref={questionTitleRef} tabIndex={-1} className="mt-8 text-2xl font-bold outline-none md:text-3xl">{question.label}</h2>
        ...
        <div className="mt-7 flex items-center justify-between gap-3">
          <button type="button" onClick={back} disabled={step === 0} className="rounded-full border border-neutral-300 px-5 py-2.5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-40 dark:border-neutral-700">
            Indietro
          </button>
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm md:p-8 dark:border-neutral-800 dark:bg-neutral-900/60">
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 ref={resultTitleRef} tabIndex={-1} className="text-sm font-semibold uppercase tracking-[0.14em] text-violet-700 outline-none dark:text-violet-300">Automation Fit Score</h2>
        ...
        <p className="mt-5 text-sm leading-relaxed text-neutral-500 dark:text-neutral-400">Il punteggio misura l’idoneità strutturale. L’esito può essere più prudente quando rischio, volume o prerequisiti attivano un gate di sicurezza.</p>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-2">
          <div className="rounded-2xl border border-neutral-200 p-5 dark:border-neutral-800">
            ...
            {result.strengths.length ? <ul className="mt-3 space-y-2 text-sm text-neutral-700 dark:text-neutral-300">{result.strengths.map((item) => <li key={item}>✓ {item}</li>)}</ul> : <p className="mt-3 text-sm text-neutral-600 dark:text-neutral-300">Non emerge ancora un vantaggio abbastanza forte da giustificare un pilot.</p>}
          </div>
          <div className="rounded-2xl border border-neutral-200 p-5 dark:border-neutral-800">
            <h3 className="font-bold">Blocchi / prerequisiti</h3>
            {result.blockers.length ? <ul className="mt-3 space-y-2 text-sm text-neutral-700 dark:text-neutral-300">{result.blockers.map((item) => <li key={item}>• {item}</li>)}</ul> : <p className="mt-3 text-sm text-neutral-600 dark:text-neutral-300">Nessun blocco strutturale evidente dalle risposte.</p>}
          </div>
        </div>
      ...
      <section className="rounded-3xl border border-violet-200 bg-violet-50/50 p-6 shadow-sm md:p-8 dark:border-violet-900 dark:bg-violet-950/20">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-2 text-2xl font-bold md:text-3xl">{result.outcome === 'GO' ? 'Il processo merita un Fit Check professionale.' : result.outcome === 'PREPARARE' ? 'Possiamo capire cosa sistemare prima del pilot.' : 'Possiamo verificare se esiste un caso d’uso migliore.'}</h2>
        ...
        {submitState === 'success' ? (
          <div role="status" className="mt-6 rounded-2xl border border-emerald-300 bg-emerald-50 p-5 text-emerald-950 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100">Richiesta inviata. Il risultato e le risposte sono stati registrati insieme al contatto.</div>
        ) : (
          <form onSubmit={submitLead} className="mt-7 grid gap-4 md:grid-cols-2" noValidate>
            <label className="grid gap-1 text-sm font-medium">Nome *<input value={contactName} onChange={(e) => setContactName(e.target.value)} className="rounded-xl border border-neutral-300 bg-white px-4 py-3 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" autoComplete="name" /></label>
            <label className="grid gap-1 text-sm font-medium">Email *<input value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-xl border border-neutral-300 bg-white px-4 py-3 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" type="email" autoComplete="email" /></label>
            <label className="grid gap-1 text-sm font-medium">Attività / azienda<input value={businessName} onChange={(e) => setBusinessName(e.target.value)} className="rounded-xl border border-neutral-300 bg-white px-4 py-3 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" autoComplete="organization" /></label>
            <label className="grid gap-1 text-sm font-medium">Telefono<input value={phone} onChange={(e) => setPhone(e.target.value)} className="rounded-xl border border-neutral-300 bg-white px-4 py-3 text-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" autoComplete="tel" /></label>
            ...
            <label className="md:col-span-2 flex items-start gap-3 text-sm text-neutral-700 dark:text-neutral-300"><input type="checkbox" checked={privacyConsent} onChange={(e) => setPrivacyConsent(e.target.checked)} className="mt-1" /><span>Ho letto la <a href="/privacy" className="font-semibold underline underline-offset-2">Privacy & Cookie Policy</a> e acconsento all’invio dei dati per essere ricontattato.</span></label>
            {formError && <p role="alert" className="md:col-span-2 text-sm font-medium text-rose-700 dark:text-rose-300">{formError}</p>}
            <div className="md:col-span-2"><button type="submit" disabled={submitState === 'sending'} className="inline-flex min-h-[48px] items-center justify-center rounded-full bg-violet-600 px-6 py-3 font-semibold text-white hover:bg-violet-700 disabled:opacity-60">{submitState === 'sending' ? 'Invio…' : 'Richiedi il Fit Check professionale'}</button></div>
          </form>
        )}
      </section>
    </div>
